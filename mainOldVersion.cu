...
//     float durationPingPongChunk;
//     GPUTimer timerPingPongChunk(0);
...
//         timerPingPongChunk.end();
//         durationPingPongChunk = timerPingPongChunk.elapsed();
//         totalDHDuration += durationPingPongChunk;

//         if(verbose) printf(">> %ld Packets (%lf GB) Transfered From Device to Host\n", (counterPing + counterPong), ((counterPing + counterPong)*sizeof(PacketBuffer))/(_GB_));
//         if(verbose) printf("\t| DurationPing : %lf ms\n", durationPingPongChunk);
//         if(verbose) printf("\t| BandwidthPing : %lf Gb/s\n", ((counterPing)*sizeof(PacketBuffer)*1000.0*8.0)/(_GB_*durationPingPongChunk));
